...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final sync = await OfflineSyncService(widget.api, _offline).flush();
      ...
      if (mounted) setState(() {
        ...
        if (cached.isEmpty) _error = 'Sem conexão e ainda não há prontuário salvo neste aparelho.';
      });
    } finally {
      ...
      body: Column(children: [
        ...
        Expanded(child: _loading
          ...
          : _error != null
              ...
              : _pets.isEmpty
                  ? Center(child: Padding(padding: const EdgeInsets.all(32), child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.pets_outlined, size: 56), const SizedBox(height: 16), const Text('Você ainda não cadastrou nenhum pet.', textAlign: TextAlign.center), const SizedBox(height: 16), FilledButton.icon(onPressed: _offlineMode ? null : _addPet, icon: const Icon(Icons.add), label: const Text('Cadastrar primeiro pet'))])))
