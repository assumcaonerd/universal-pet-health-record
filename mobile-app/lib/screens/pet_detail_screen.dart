...
class _PetDetailScreenState extends State<PetDetailScreen> {
  ...
  Future<void> _deactivateAllergy(Allergy allergy) async {
    final confirmed = await showDialog<bool>(context: context, builder: (context) => AlertDialog(title: const Text('Marcar alergia como inativa?'), content: Text('O registro de ${allergy.allergen} continuará no histórico, mas deixará de aparecer como alergia ativa.'), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancelar')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Desativar'))]));
    if (confirmed != true) return;
    try { await _clinical.deactivateAllergy(widget.pet.id, allergy.id); await _loadAllergies(); if (mounted) setState(() {}); } on ApiException catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message))); }
  }
  ...
  Future<void> _delete() async {
    ...
    if (confirmed != true) return;
    try { await widget.api.delete('/pets/${widget.pet.id}'); if (mounted) Navigator.of(context).pop(true); } on ApiException catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message))); }
  }
